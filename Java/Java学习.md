...
![](https://md-file-zhaowei.oss-cn-beijing.aliyuncs.com/Java%E5%AD%A6%E4%B9%A0/1747126429488.jpg)
...
- 1、PROPAGATION REQUIRED:如果当前没有事务，就创建一个新事务，如果当前存在事务就加入该事务，该设置是最常用的设置。
...
- 4、PROPAGATION REQUIRES NEW:创建新事务，无论当前存不存在事务，都创建新事务。5、PROPAGATION NOT_SUPPORTED:以非事务方式执行操作，如果当前存在事务，就把当前事务挂起。
- 6、PROPAGATION NEVER:以非事务方式执行，如果当前存在事务，则抛出异常。
- 7、PROPAGATION NESTED:如果当前存在事务，则在嵌套事务内执行。如果当前没有事务，则执行与 PROPAGATION REQUIRED 类似的操作。
...
//但是注意想实现不同事务的传播模型的实现需要由不同的类进行实现，如果上面的函数都是一个类中的方法，那么其实都没有作用，这是因为事务是由代理来进行实现的，如果在同一个类中则只有一个代理进行操作
//本地事务失效问题
//同一个对象内事务方法互调默认失效，原因 绕过了代理对象，事务使用代理对象来控制的
//解决:使用代理对象来调用事务方法
//1)、引入aop-starter;spring-boot-starter-aop;引入了aspectj
//2)、@EnableAspectIAutoProxy(exposeProxy= true);开启 aspectj 动态代理功能。以后所有的动态代理都是aspectj创建的（即使没有接口也可以创建）
//对外暴露代理对象
//3)、本类互调用调用对象OrderServiceImpl orderService =(OrderServiceImpl)AopContext.currentProxy();
//orderService.b();
//orderService.c();
```

![](https://md-file-zhaowei.oss-cn-beijing.aliyuncs.com/Java%E5%AD%A6%E4%B9%A0/1747127443637.jpg)

**CAP定理和raft算法**

CAP定理指出，在分布式系统中，一致性（Consistency）、可用性（Availability）和分区容错性（Partition Tolerance）三者不可兼得，最多只能同时满足两项。当网络分区发生时，系统需要在一致性和可用性之间做出选择：若优先保证一致性，则部分节点可能无法响应请求（牺牲可用性）；若优先保证可用性，则可能返回旧数据（牺牲一致性）。CAP定理为分布式系统设计提供了理论指导，帮助在不同场景下权衡核心需求。

Raft是一种分布式一致性算法，通过选举领导者（Leader）实现多节点间的状态一致性。其核心流程包括领导者选举、日志复制和安全性保证。所有客户端请求由Leader处理，日志需复制到多数节点后才能提交，确保数据一致性。若Leader故障，剩余节点通过投票选举新Leader，避免单点问题。Raft通过逻辑分离和清晰的状态机制，降低了共识算法的复杂度，广泛应用于Etcd、Consul等系统，一共俩种超时时间，第一种是随从变候选者的等待时间，第二种是领导向随从者发送日志的超时时间。

**BASE理论以及强一致性、弱一致性和最终一致性**

BASE理论是对CAP中一致性与可用性权衡的实践延伸，包含**基本可用（Basically Available）**、**软状态（Soft State）**和**最终一致性（Eventually Consistent）**。基本可用指系统在故障时仍能提供降级服务；软状态允许中间态存在，不要求实时一致；最终一致性则保证数据最终一致。BASE弱化对强一致性的要求，通过异步和柔性设计实现高可用，适用于电商、社交等容忍短暂不一致的场景。

- **强一致性**：任何读写操作都能立即获取最新数据，如分布式事务提交后的状态。
- **弱一致性**：不保证后续访问能立即读到最新值，允许数据存在短暂不一致。
- **最终一致性**：弱一致性的特例，确保在无新更新的情况下，数据经过一定时间后达成一致。例如，DNS更新和分布式存储系统常采用此模型。

**分布式事务解决办法**

- 2PC模式  2PC是经典的分布式事务协议，包含**准备阶段**和**提交阶段**。协调者先询问所有参与者能否提交事务（投票），若全部同意则发送提交指令，否则回滚。其缺点是同步阻塞（参与者资源锁定）、协调者单点故障，以及潜在的数据不一致（如协调者崩溃后参与者状态未知）。尽管简单，但因可靠性问题，逐渐被改进方案替代。
- 柔性事务-TCC事务补偿型方案  
- TCC（Try-Confirm-Cancel）通过业务逻辑分解实现柔性事务：**Try**：预留资源（如冻结库存）。**Confirm**：确认操作（如扣减库存），需保证幂等性。**Cancel**：补偿回滚（如释放库存）。TCC要求业务方自定义补偿逻辑，适用于高一致性需求场景（如金融交易），但开发复杂度较高。
- 柔性事务-最大努力通知型方案  事务发起方通过多次重试通知参与方处理，直至成功或达到阈值后人工介入。例如支付成功后通知商户系统，允许最终失败但需记录日志。该方案依赖消息可靠性，适用于对一致性要求较低的场景（如状态通知），需结合对账机制保证最终一致。
- 柔性事务-可靠消息+最终一致性方案  通过消息队列（如RocketMQ事务消息）确保消息可靠传递，实现跨服务数据最终一致。例如订单服务创建订单后发送消息，库存服务消费消息扣减库存。核心机制包括本地消息表（保障消息持久化）和消费者重试（处理失败场景），适用于异步解耦的高并发场景。

**Alibaba Seata实现分布式事务**  Seata是开源的分布式事务解决方案，支持AT、TCC、Saga等模式：

- **AT模式**：自动生成反向SQL回滚日志（undo_log），通过全局锁保证隔离性，对业务侵入低。
- **TCC模式**：需手动实现Try/Confirm/Cancel接口，适用于复杂业务补偿。
- **Saga模式**：通过事件驱动和补偿机制处理长事务，适合跨多服务的业务流程。
  Seata通过TC（事务协调器）管理全局事务状态，协调各RM（资源管理器）实现最终一致性，广泛应用于微服务架构。

**分布式事务的应用** 在我们的项目中会遇到各种各样与数据库打交道的场景，但是不同场景下我们的操作又不太一样，但我们不需要进行高并发操作的时候如何实现BASE理论呢，我们可以使用Seata实现分布式事务，也就是可以通过柔性事务-事务补偿型方案来进行实现，在seate中通过undolog以及全场景管理器就可以实现分布式事务，但是这特别消耗时间，虽然达到了BASE理论，但是对并发场景有要求。那么在高并发的场景中怎么实现分布式事务呢，我们可以通过实现柔性事务-可靠消息+最终一致性方案这样在高并发的场景下也能实现分布式事务的应用。

![](https://md-file-zhaowei.oss-cn-beijing.aliyuncs.com/Java%E5%AD%A6%E4%B9%A0/1747143970787.jpg)

**RabbitMQ实现分布式事务：** 实际上在高并发的场景下实现分布式事务主要通过延迟队列进行实现，而这种延迟队列可以通过spring的schedule定时任务进行实现，但是这种方式会出现过多等待的情况发生，比如确定了定时30s当任务在第一秒进行提交并在之后的第30入队，时间会到达1分钟的时候才被察觉，消息任务进行了等待59秒大大增加了任务处理时间。所以我们提出通过消息队列实现，在这里我们主要通过消息的TTL和死信Exchange进行结合实现。

![](https://md-file-zhaowei.oss-cn-beijing.aliyuncs.com/Java%E5%AD%A6%E4%B9%A0/1747154117772.png)

![](https://md-file-zhaowei.oss-cn-beijing.aliyuncs.com/Java%E5%AD%A6%E4%B9%A0/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20250514090626.png)

在本项目中使用以上的方式实现延迟队列，这样可以仅仅使用一个交换机即可。实现之后我们的业务就可以进行实现高并发情况下的分布式事务了，一般情况下有一下几种情况需要进行安排分布式业务处理即，1、下订单成功，订单过期没有支付被系统自动取消、被用户手动取消。都要解锁库存。2、下订单成功，库存锁定成功，接下来的业务调用失败，导致订单回滚，之前锁定的库存就要自动解锁。这两种状态都需要通过延迟队列进行实现，同时还需要对正常业务进行区分，所以还需要通过和数据库存储的记录进行对比，即在调用其他事务操作的时候进行存储订单事务表和仓库表的对应关系，这样在最后接受到消息队列的时候进行查看数据库中是否存在仓库数据

- 若不存在那么就是该本地事务本身出现问题进行回滚了，无需进行处理
- 若存在，那么需要进行查看数据中的订单记录是否存在，若不存在那就说明后续的业务出现问题，综合事务进行回滚但是仓库事务正常所以为了最终一致性就需要进行操作回滚
- 若仓库记录存在同时订单记录也存在，这个时候就需要进行查看订单状态，是否有异常，若有那么也是需要进行操作回滚才能保持最终一致性。同时若没有异常则就说明这个过程中没有事务回滚同时业务状态正常，那么就可以无需进行处理了

**注意：**这里设置完消息队列之后还需要查看他的监听服务是否是默认的，在默认情况下一旦接收到消息，那么消息就被移除，不会因为错误而让下一次业务在消息队列中进行查看。

### 秒杀服务

**秒杀业务** 秒杀具有瞬间高并发的特点。针对这一特点，必须要做限流+异步+缓存(页面静态化)+独立部署。**限流方式**有以下几种：

- 1.前端限流，一些高并发的网站直接在前端页面开始限流，例如，小米的验证码设计2.nsinx限流，直接负载部分请求到错误的静态页面:令牌算法 漏斗算法
- 3.网关限流，限流的过滤器
- 4.代码中使用分布式信号量
- 5.rabbitmq限流(能者多劳:chanel.basicQos(1))，保证发挥所有服务器的性能

**定时任务** 在秒杀任务中我们商品的上架往往是在秒杀时间之前进行上架，而这个时候一般不将数据直接上传到数据库中导致数据库中的压力过大，那么就需要使用中间件redis，同时也需要进行定时任务的实现，在本项目中我们通过**QUARTZ的job scheduler以及cron表达式**进行实现，但是需要注意的就是秒杀活动项目中定时服务会多个进行运行，**而定时服务又是应该和现实时间所对应不应该被阻塞**，所以就需要将这些多个定时服务线程进行**异步编排**，从而实现我们的业务效果，这里可以使用SpringBoot的@EnableScheduling和@Async这两个注解进行AOP简单实现从而避免大量复杂操作。

**秒杀商品上架操作** 在这个部分首先我们需要将数据进行收集然后为秒杀服务提供接口进行调用，这样就可以在秒杀服务中进行上传商品信息，在此之前我们有以下几个步骤

- 首先需要在优惠服务中进行查找所有满足最近三天的优惠信息活动，并将这些活动按照List的形式进行返回，同时将这些活动中所包含的所有商品进行包装与活动信息一起进行返回，为秒杀服务提供基本的信息源。
- 然后需要在秒杀服务中进行操作，首先需要编写秒杀服务中的Serive层的实现，这样在定时任务的时候就可以通过调用Serive层进行操作，这样的话我们可以先通过完成配置类然后再创建一个定时操作，在定时操作的时候进行调用服务类，这样就实现了定时扫描需要进行上架的商品信息，并进行保存在redis中准备上架。
- 创建秒杀服务中的Serive层的代码，我们需要通过远程调用优惠服务的功能实现将**活动场次id+sku详细信息(SkuInfoVo)+sku秒杀信息(秒杀价格、总量、开始结束时间)+秒杀随机码**全部提交到redis中进行保存，**注意：**这里有关于活动和商品的唯一信息需要通过Hash表的数据结合进行在redis中存储，既可以提高查询速度，也避免了重复数据的保存，这是因为可能未来会有多个服务器进行同时定时操作，使用hash就不会出错，同时此处添加秒杀随机码的目的是**避免恶意争抢**，确保是公平的秒杀，只有在秒杀开始时才可以秒杀，如果不加随机码，直接带skuId的话就会有被脚本恶意攻击的可能。**分布式信号量：**seckill:stock:#(商品随机码)，在使用库存作为分布式信号量时也是使用的这个随机码，避免被伪造的恶意请求把信号量给减去了。
- 实现以上操作仅仅只是将页面的商品信息进行保存在redis中了，这样当秒杀时间到了就可以通过定时操作从redis中进行快速取出展示，但是我们其实还需要为将来的高并发进行考虑，因为展示商品是为了能够去进行下单支付，那么衡量用户是否秒杀成功就是看能不能将库存数据进行修改，而数据库毕竟是有限的资源，不能将所有的请求都放到数据库中进行修改数据，那么我们可以在redis中进行初步判断，哪些用户成功修改，也就是将库存信息放入redis中，让用户进行秒杀抢购，但是这个数据资源明显是需要抢夺的临界资源，并且还存放在redis中，那么这个操作就是**准备分布式信号量**，通过分布式锁实现对访问共享资源的同步。
- **最后检查幂等性原则**  幂等性意味着无论调用多少次，系统的状态或结果都不会发生变化。 这里主要是在进行集群分布式上架操作的时候进行**加分布式锁**，以及进行**检索之前存储的hash数据**，若redis中已经存在那么就不进行保存，反之进行保存redis。同时在保存随机码以及**分布式信号量的时候也要进行判断**，在这个部分由于不是存储hash值，那么就需要通过之前的保存的场次信息或者商品信息进行绑定然后进行判断，这样在一个锁中就可以实现有没有已经保存过信息，从而实现幂等性。

![](https://md-file-zhaowei.oss-cn-beijing.aliyuncs.com/Java%E5%AD%A6%E4%B9%A0/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20250514090628.png)

**查看并展示秒查产品** 前者只是定时将数据商城上传到redis中，还需要一个controller来实现如何查询，查询部分就只需要将当前时间保存遍历所有的秒杀上架服务，查看当前时间是否在秒查商品的场次时间内，如果是则进行返回，反之就不用。由于查看数据会返回list数据，展示效果就在页面中进行访问这个函数的返回结果，保存并遍历数据在页面的每个框中进行展示即可。

**秒杀系统**  一般的秒杀系统分为两种，第一种为秒杀系统就是正常的下单系统，只不过是将秒杀信息的价格与优惠系统进行结合得出更加便宜的商品，用户进行下单即可。第二中就是为秒杀系统单独作为一个微服务，用户使用这个微服务进行秒杀操作，成功秒杀后再通过消息队列进行处理订单业务。

![](https://md-file-zhaowei.oss-cn-beijing.aliyuncs.com/Java%E5%AD%A6%E4%B9%A0/9c4ac221572eb7ffbb81f38fe714e513.png)

![](https://md-file-zhaowei.oss-cn-beijing.aliyuncs.com/Java%E5%AD%A6%E4%B9%A0/c2981ef357a2d292b011593340352230.png)

**注意：** 前者的好处就是业务简单，同时流量会分到各个微服务中，但是一旦系统出现灾难性错误无法进行有效的解耦进行处理，后者的好处就是业务独立，同时也为高并发进行了有效的应对，在生成订单之前都不会进行远程调用和访问数据库，但是如果在流程的后续消息队列中出现错误，就会导致消息无法处理，业务迟迟没有反应，这个时候可能需要在微服务的后段加上自己的业务处理。

**秒杀过程** 本项目使用的是第二种方式进行实现秒杀过程，在这个框架中可以尽可能的实现高并发的效果，首先为我们的秒杀页面进行前端准备，这里可以通过像之前的动静分离的方式进行处理从而减少流量的进入。

- 检查登录，秒杀操作一定是要登录的所以我们在前端的秒查请求之前需要进行判断是否登录，通过调用登录微服务实现验证登录，如果登录则进行后续的秒杀服务，若没有的话就进行跳转登录页面，用户选择进行登录。
- 合法性校验，验证登录信息之后，系统需要在秒杀强占之前进行验证秒查操作的合法性，在秒杀服务中依次进行检查**是否存在商品**、**当前时间是否具有时效性**、**验证商品和随机码是否与redis中的存储匹配**、**检查用户的秒查次数和商品允许的秒杀次数是否匹配**以及**进行查看库存信号量是否还支持秒杀**，到这里用户基本实现了合法性的检查，同时为了高并发的效果，这些步骤都没有进行加锁的操作，这是因为都是读操作。
- 获取信号量，这个位置就是需要进行写入数据的操作，所以通过redis的分布式锁实现分布式信号量来进行秒杀，系统首先进行对信号量的读取，同时验证是否读取到，若用户成功读取到数据则说明该用户实现了秒杀的操作，这个时候系统就**开始进行创建订单号**、**创建秒杀订单信息**以及**将创建秒杀订单的消息发送到消息队列中**，当然最后的时候会进行对redis中的库存信号量进行修改，这个时候就会直接返回用户提示秒杀成功。这样通过消息队列实现削峰填谷可以使跟多的流量进来，同时我觉着可以实现延迟队列从而保证了数据的最终一致性。
- 用户得到秒杀成功的消息会去进行确认商品购买信息以及进行支付，而这个操作就可以不在秒杀服务中进行，通过**消息队列在订单微服务中进行监听消息队列**，当用户的订单接收到了之后，系统会进行检查用户操作的最终正确性，通过订单微服务中的各种操作为用户真正实现秒杀的全部过程。

### 高并发

在秒杀服务中通过设置秒杀微服务实现独立部署，从而单一节点出现问题就可以进行快速反应并处理

在秒杀服务中通过设置随机码和商品id或者进行加密，从而实现了防止脚本语言进行大量秒查操作

在秒杀服务中通过在redis中存放商品数据以及秒杀库存大小信息，从而实现了快速扣减以及实现信号量操作

在秒杀服务中通过在nginx中存放静态数据资源，从而用户多次访问商品系统的时候对于前端数据和部分数据就仅仅需要一次访问

![](https://md-file-zhaowei.oss-cn-beijing.aliyuncs.com/Java%E5%AD%A6%E4%B9%A0/1746487530925.jpg)

在秒杀服务中通过前端检查以及日志记录，还有在redis层中设置布隆过滤器或者设置空的键值对进行拦截数据，从而减低服务器压力

在秒杀服务中通过设置验证码或者设置登录验证，从而实现流量错峰的效果

在秒杀服务中通过前端限流设置连续点击次数以及后端限流实现分布式锁、熔断降级等，从而实现系统的高并发的效果

在秒杀服务中通过消息队列将业务的部分流程以消息的方式发送给其他微服务进行处理，从而实现了高并发中可以接收住大量请求压力
...
虚拟机接口，nacos设置，redis接口（有俩个），线程池设置，oss远程上传配置，linux使用，docker进行下载，部署nginx，ssh传输文件，使用nginx，sleuth进行分布式链路追踪zipkin进行可视化，sleuth在springboot项目中引入然后在每次调用时候进行使用，来追踪分布式调用链路的数据追踪，而zipkin是一款软件我们将其安装在nginx的服务器的docker中（注意版本2.12.3才可以访问web），然后通过接口来进行管理使用，rabbitmq的配置，以及确认机制的配置
...
面试官你好，我叫赵伟，今年25岁，现在就读在北京交通大学计科专硕，目前研一，本人有扎实的Java基础，良好的编程风格，熟悉jvm虚拟机基本原理，内存分配机制、垃圾回收算法等。熟悉mysql关系型数据库的使用和原理，比如事务隔离级别和实现原理，索引等。掌握像SSM这样的框架之间的整合，以及一些相关组件的使用，比如Docker、redis、Nginx、es、git 以及mq等等。了解分布式系统的设计和应用，缓存、消息队列、负载均衡等机制和实现。在学习的过程中呢，我也对这些技术在自己的项目上进行了相应的应用，尝试优化实际业务中可能出现的问题。热爱代码，对技术有不懈的追求。非常希望能加入贵公司团队担任后端开发实习生，以上就是我的自我介绍。
...
**http请求：**HTTP请求是客户端（如浏览器、移动应用等）向服务器发送的一种通信消息，用于获取或提交数据。HTTP（HyperText Transfer Protocol）是互联网上应用最广泛的网络协议之一。HTTP请求的基本结构包含以下几个部分：

- 请求行（Request Line） 包含三个要素分别为HTTP方法（如GET、POST、PUT、DELETE等）、请求的URL路径以及HTTP版本（如HTTP/1.1、HTTP/2）
- 请求头（Headers） 包含关于请求的元信息，比如Host：目标服务器地址、User-Agent：客户端信息、Content-Type：请求体的数据类型以及Authorization：身份验证信息等

- 请求体（Body） 包含要发送给服务器的数据，主要用于POST、PUT等方法。
...
HTTP请求是现代web应用和API通信的基础，几乎所有的网络交互都基于HTTP协议。

**GET与POST的区别：**一般get和post请求类型是在http请求中最为常见的，他们有以下三点区别
...
**session、cookie和token的区别：** 首先我们需要知道http是一个无状态协议，什么是无状态呢？就是说这一次请求和上一次请求是没有任何关系的，互不认识的，没有关联的。这种无状态的的好处是快速。坏处是假如我们想要把www.zhihu.com/login.html和www.zhihu.com/index.html关联起来，必须使用某些手段和工具。由于http的无状态性，为了使某个域名下的所有网页能够共享某些数据，session和cookie出现了，需要首先知道的几点：
...
所以使用session+cookies的流程就是，首先，客户端会发送一个http请求到服务器端。服务器端接受客户端请求后，建立一个session，并发送一个http响应到客户端，这个响应头，其中就包含Set-Cookie头部。该头部包含了sessionId。在客户端发起的第二次请求，假如服务器给了set-Cookie，浏览器会自动在请求头中添加cookie服务器接收请求，分解cookie，验证信息，核对成功后返回response给客户端，简而言之, session 有如用户信息档案表, 里面包含了用户的认证信息和登录状态等信息，而 cookie 就是用户通行证。

那么什么是token呢，token 也称作令牌，由uid+time+sign[+固定参数]，token 的认证方式类似于临时的证书签名, 并且是一种服务端无状态的认证方式, 非常适合于 REST API 的场景. 所谓无状态就是服务端并不会保存身份认证相关的数据。所以token 的认证流程与cookie很相似，那么他们的区别是什么呢：

- cookie需要服务器进行维护，token不需要

  这是因为cookie实际上是在请求头中保存了一个session的id，服务器收到后会解析id得到相关的会话信息，而token是讲所有的信息进行编码然后放入请求头中，服务器得到token直接进行解析就可以得到相关信息
...
**什么是JWT：**由于cookie是执行的同源策略，所以在分布式的情况下就会遇到负载均衡的问题，而token是无状态的，token字符串里就保存了所有的用户信息，客户端登陆传递信息给服务端，服务端收到后把用户信息加密（token）传给客户端，客户端将token存放于localStroage等容器中。客户端每次访问都传递token，服务端解密token，就知道这个用户是谁了。通过cpu加解密，服务端就不需要存储session占用存储空间，就很好的解决负载均衡多服务器的问题了。这个方法叫做[JWT(Json Web Token)](https://link.segmentfault.com/?enc=Syemw%2F1paZKjAZ0tjFEDWQ%3D%3D.GYlFz9Z%2F%2B%2FNKVxe5vkoc5T5TVGA%2FcWs62UFUv7KKEp1SI52n49rtjX4H4h925F%2Fe6pvx4BhyNyrxoPK%2Fci1wRQ%3D%3D)

**使用postman进行请求测试：** 在postman中可以自己定义请求体、请求头和请求行，并且还特别的把请求行中添加参数的操作通过Param提供出来，以及把请求头中的Authorization属性添加也提取出来，同时还提供了执行前的脚本操作和设置，让开发者更加轻松的进行测试
...
15. "使用nginx的原因是什么？比较好的原因？"
16. "看你用seata解决分析问题，还使用了RabbitMQ，这里具体事务的话是怎么用的？"
17. "seata支持哪些事务模式？"
18. "这个具体是怎么使用的呢？就是在这个场景下？"
19. "使用RabbitMQ去做可靠消息保证最终一致性，这个怎么实现的？"
20. "具体是怎么用的？为什么能保证这种一致性？"
21. "这些是怎么体现的呢？看起来这个流程里面没有体现出事务的特征对吧？事务提交哪个地方体现出了事务的一些特征？"
22. "用过其他的一些中间件吗？"

**项目运行相关**

23. "这个项目平时能跑吗？"
24. "写Redis作为中间件提升系统性能，这个具体是做什么？"
25. "这个商品服务实现那些什么接口？提供什么接口？"
26. "这个搜索是怎么实现的呢？你讲一下"
27. "范围是什么含义呢？这个价格的时候去查询有什么关系吗？"
28. "实现了OAuth的社交登录，那个在哪一块？事项需要登录？"
29. "登录原理是什么？用哪种模式？"

**问题排查**

30. "如果现在你的那个前台有个页面，后端的这个某个进程HTTP的一个接口超时了，你要去排查，怎么排查？"

**Java基础知识**

31. "一个Boolean在内存里面占多大的内存？"
32. "就只有一个变量，变量只有一个布尔的那个变量参数"

**算法题**

33. "算法题，链表里面有没有环？"
34. "为什么说快慢指针走到一起的时候，让指针从头进行遍历，然后再次进行重合的位置那就是环的起点？"
35. "现在写一下"
36. "怎么把这个链表构建起来？"
37. "这个列表怎么构建？你怎么把这个列表构建起来？"
38. "怎么写呢？"
39. "运行一下怎么做？"

**结束语**

40. "这个你下来再调一下，我估计差不多也好，应该差不多了"

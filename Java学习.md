...
**定时任务** 在秒杀任务中我们商品的上架往往是在秒杀时间之前进行上架，而这个时候一般不将数据直接上传到数据库中导致数据库中的压力过大，那么就需要使用中间件redis，同时也需要进行定时任务的实现，在本项目中我们通过**QUARTZ的job scheduler以及cron表达式**进行实现，但是需要注意的就是秒杀活动项目中定时服务会多个进行运行，**而定时服务又是应该和现实时间所对应不应该被阻塞**，所以就需要将这些多个定时服务线程进行异步编排，从而实现我们的业务效果，这里可以使用SpringBoot的@EnableScheduling和@Async这两个注解进行AOP简单实现从而避免大量复杂操作。
